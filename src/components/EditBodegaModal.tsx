...
export default function EditBodegaModal({ bodega, onClose, onSuccess }: EditBodegaModalProps) {
    const [nombre, setNombre] = useState(bodega.nombre);
    const [ubicacion, setUbicacion] = useState(bodega.ubicacion);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setLoading(true);
            setError('');
            await bodegas.update(bodega.id, {
                nombre: nombre.trim(),
                ubicacion: ubicacion.trim(),
            });
            onSuccess();
        } catch (err: any) {
            console.error('Error updating bodega:', err);
            setError(err.response?.data?.detail || 'Error al actualizar la bodega');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <Modal isOpen={true} onClose={onClose} title="Editar Bodega">
